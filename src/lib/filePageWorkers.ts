...
export const DEFAULT_FILE_PAGE_WORKER_MODE: FilePageWorkerMode = 'sort-data';
...
const WORKER_FOCUS_META: Record<FilePageWorkerFocus, WorkerFocusMeta> = {
  general: {
    label: 'General',
    promptLabel: 'general source pack',
    promptInstructions: [
      'Create a balanced pack with concise summary, key facts, entities, and next-useful details.',
      'Optimize for quick orientation without reading the full source.',
    ],
  },
  coding: {
    label: 'Coding',
    promptLabel: 'coding-oriented source pack',
    promptInstructions: [
  ...
  describing: {
    label: 'Describing',
    promptLabel: 'descriptive source pack',
    promptInstructions: [
  ...
  research: {
    label: 'Research',
    promptLabel: 'research-oriented source pack',
    promptInstructions: [
...
export function resolveWorkerMode(mode: FilePageNode['workerMode']): FilePageWorkerMode {
  return mode === 'ai-ready' ? 'sort-data' : mode ?? DEFAULT_FILE_PAGE_WORKER_MODE;
}
